...
Elm.Chess.Game.make = function (_elm) {
   ...
   origin) {
      return function () {
         ...
         8) ? {ctor: "_Tuple2"
              ,_0: _L.fromArray([])
              ,_1: $Maybe.Just(Promotion(nextPosition))} : function () {
            var _v15 = game.state;
            switch (_v15.ctor)
            {case "Origin":
               switch (_v15._0.ctor)
                 {case "Just":
                    return _U.eq(_v15._0._0,
                      left) || _U.eq(_v15._0._0,
                      right) ? function () {
                         var enPassantDestination = $Chess$Board.shift(_v15._0._0)(function () {
                            var _v18 = game.turn;
                            switch (_v18.ctor)
                            {case "Black":
                               return {ctor: "_Tuple2"
                               ...
                               return {ctor: "_Tuple2"
                                      ...
                                      ,_1: 1};}
                            _U.badCase($moduleName,
                            "between lines 271 and 277");
                         }());
                         return {ctor: "_Tuple2"
                                ,_0: _L.fromArray([enPassantDestination])
                                ,_1: $Maybe.Just(EnPassant(enPassantDestination))};
                      }() : noSpecialMove;}
                 break;}
   ...
   piece) {
      return function () {
         var destinationHasNoAlly = function (destination) {
            return function () {
               var _v20 = A2($Chess$Board.getSquareContent,
               game.board,
               destination);
               switch (_v20.ctor)
               {case "Just":
                  return !_U.eq(_v20._0.color,
                    game.turn);
                  case "Nothing": return true;}
               _U.badCase($moduleName,
               "between lines 401 and 407");
            }();
         };
